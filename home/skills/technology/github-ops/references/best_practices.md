...
# Use gh api when you need explicit page control
gh api repos/{owner}/{repo}/pulls -f per_page=100 -f page=1
gh api repos/{owner}/{repo}/pulls -f per_page=100 -f page=2
```

### Processing Large Result Sets

```bash
# Get all PRs in batches via the REST API
for page in {1..10}; do
  gh api repos/{owner}/{repo}/pulls -f per_page=100 -f page="$page"
done

# Stop when no more results
page=1
while true; do
  results=$(gh api repos/{owner}/{repo}/pulls -f per_page=100 -f page="$page")
...
# Use with an explicit enterprise repository target
gh pr list --repo github.enterprise.com/OWNER/REPO
...
GH_HOST=github.enterprise.com GH_TOKEN=ghp_enterprise_token gh pr list
```
...
# Check CI status
checks=$(gh pr checks 123 --json bucket -q '.[].bucket')
if echo "$checks" | grep -qx 'fail'; then
...
while true; do
  buckets=$(gh pr checks "$PR_NUM" --json bucket -q '.[].bucket')
  if [ -n "$buckets" ] && ! echo "$buckets" | grep -Eq '^(fail|pending|cancel)$'; then
...
gh config set git_protocol ssh --host github.com  # or https
...
```bash
# Create PR
PR_URL=$(gh pr create --title "Feature" --body "Description")
PR_NUM="${PR_URL##*/}"

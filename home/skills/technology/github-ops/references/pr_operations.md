...
```bash
# Create PR with a repository-appropriate title
gh pr create --title "Your PR title" --body "PR description"

# Create PR with ticket reference when the repo expects one
gh pr create --title "ABC-123: Your PR title" --body "PR description"
...
gh pr create --title "Feature" --body-file pr-description.md
```

### PR Title Convention

- Follow repository-specific title rules before creating the PR.
- Common patterns include ticket-prefixed titles like `ABC-123: Descriptive title`, conventional titles like `feat: add import command`, or plain descriptive titles.
...
# Get checks as JSON
gh pr checks 123 --json name,state,workflow,link
```
